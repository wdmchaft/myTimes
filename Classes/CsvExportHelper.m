...
+(NSString*) getCsvExportData:(NSArray*)projects {
	...
	for (Project* p in projects) {
		if (p.markedForExport) {
			...
			float projectTotalAmmount = [[p totalAmmountMarkedForExport] floatValue];
			NSLog(@"project total ammount: %@", [CsvExportHelper formatSeconds:projectTotalAmmount]);
			//description 
			...
			if (tasks != nil) {
				for (ProjectTask* task in tasks) {
					if (task.markedForExport) {
						...
						float taskTotalAmmount = [[task totalAmmountMarkedForExport] floatValue];
						//description 
						...
						if (workUnits != nil) {
							for (TimeWorkUnit* wu in workUnits) {
